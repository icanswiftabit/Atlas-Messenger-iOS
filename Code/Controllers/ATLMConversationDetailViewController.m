...
@property (nonatomic) BOOL currentUserIsAdmin;
...
NSString *const ATLMDeleteTerminalGroupText = @"Delete Terminal Group";
...
- (id)initWithConversation:(LYRConversation *)conversation withLayerController:(ATLMLayerController *)layerController
{
    ...
    if (self) {
        ...
        _currentUserIsAdmin = layerController.layerClient.authenticatedUser.isAdmin;
    }
    return self;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section) {
        ...
        case ATLMConversationDetailTableSectionParticipants:
            return [self canEditConversationDetails] ? self.participants.count + 1 : self.participants.count;
            
        case ATLMConversationDetailTableSectionLocation:
            return 1;
            
        case ATLMConversationDetailTableSectionLeave:
            return self.conversation.terminalGroup ? 0 : 1;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        ...
        case ATLMConversationDetailTableSectionLeave: {
            ...
            if (self.conversation.terminalGroup) {
                cell.centerTextLabel.text = ATLMDeleteTerminalGroupText;
            } else {
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    switch ((ATLMConversationDetailTableSection)section) {
        case ATLMConversationDetailTableSectionMetadata:
            return self.conversation.terminalGroup ? @"Terminal Group Name" : @"Conversation Name";
            
        case ATLMConversationDetailTableSectionParticipants:
            return self.conversation.terminalGroup ? @"Assigned Users" : @"Participants";
...
- (void)configureConversationNameCell:(ATLMInputTableViewCell *)cell
{
    ...
    cell.textField.enabled = self.conversation.terminalGroup ? NO : [self canEditConversationDetails];
    cell.guideText = @"Name:";
    cell.placeHolderText = self.conversation.terminalGroup ? @"Enter Terminal Group Name" : @"Enter Conversation Name";
    NSString *conversationName = [self.conversation.metadata valueForKey:ATLMConversationMetadataNameKey];
    NSString *terminalGroup = [self.conversation.metadata valueForKey:HONTerminalNameMetadataNameKey];
    cell.textField.text = self.conversation.terminalGroup ? terminalGroup : conversationName;
}
...
{
    NSString *addParticipantString = self.conversation.terminalGroup ? @"Add Assigned User" : @"Add Participant";
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:addParticipantString];
...
- (void)confirmLeaveConversation
{
    ...
    if (self.conversation.terminalGroup) {
        destructiveButtonTitle = ATLMDeleteTerminalGroupText;
    } else {
        destructiveButtonTitle = self.conversation.participants.count > 2 ? ATLMLeaveConversationText : ATLMDeleteConversationText;
    }
...
- (BOOL)canEditConversationDetails
{
    return (!self.conversation.terminalGroup || (self.conversation.terminalGroup && self.currentUserIsAdmin));
}
