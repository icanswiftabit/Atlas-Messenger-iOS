...
@interface ATLMConversationListViewController () <ATLConversationListViewControllerDelegate, ATLConversationListViewControllerDataSource, UIActionSheetDelegate>
...
+ (instancetype)conversationListViewControllerWithLayerController:(ATLMLayerController *)layerController withFolderName:(NSString *)folderName
{
    NSAssert(layerController, @"Layer Controller cannot be nil");
    return [[self alloc] initWithLayerController:layerController withFolderName:folderName];
}
...
- (instancetype)initWithLayerController:(ATLMLayerController *)layerController withFolderName:(NSString *)folderName
{
    NSAssert(layerController, @"Layer Controller cannot be nil");
    self = [self initWithLayerClient:layerController.layerClient];
    if (self)  {
...
- (void)viewDidLoad
{
    self.shouldDisplaySearchController = NO;
...
- (LYRQuery *)conversationListViewController:(ATLConversationListViewController *)viewController willLoadWithQuery:(LYRQuery *)defaultQuery
{
    ...
    if (self.folderName) {
        query.predicate = [LYRPredicate predicateWithProperty:[NSString stringWithFormat:@"metadata.%@", HONFolderNameMetadataNameKey] predicateOperator:LYRPredicateOperatorIsEqualTo value:self.folderName];
...
- (NSString *)conversationListViewController:(ATLConversationListViewController *)conversationListViewController titleForConversation:(LYRConversation *)conversation
{
    ...
    NSString *conversationTitle = conversation.terminalGroup ? conversation.metadata[HONTerminalNameMetadataNameKey] : conversation.metadata[ATLMConversationMetadataNameKey];
...
- (NSString *)conversationListViewController:(ATLConversationListViewController *)conversationListViewController lastMessageTextForConversation:(LYRConversation *)conversation {
    ...
    if([part.MIMEType isEqualToString:HONMIMETypeTerminalStatus]) {
        NSString *rating = @"95";
        return [NSString stringWithFormat:@"Terminal Status: %@", rating];
    } else if ([part.MIMEType isEqualToString:HONMIMETypeTerminalReport]) {
        return @"Weekly Report Received";
    }
    return nil;
}
